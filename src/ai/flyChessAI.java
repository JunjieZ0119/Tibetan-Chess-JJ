...
public class flyChessAI {
	public Core core;//棋盘
	int Walk_AI_STEP = 16; //向下模拟步数
	int Walk_AI_ALL = 100000;//总随机次数 100000
	int EAT_AI_ALL  = 50000;//吃子模拟盘数 50000
	...
	public List<CanWalkChess> get_NO_CanWalkChessList(int var,int[][] core){
		...
		List<CanWalkChess> canMove_list = new ArrayList<CanWalkChess>();
		int get_ge = 0;
		for(int i=0;i<14;i++){//终点选择
			for(int j=0;j<14;j++){
				CanWalkChess canflyChess = new CanWalkChess();
				int end_value=0;
				int eat_num=0;
				if(core[i][j]==0){//空点，就是可落点，是终点
					...
					}else if(end_value>=1){//减去不沾边的点
						canMove_list.add(canflyChess);
					}
				}
			}
		}
		for(int i=0;i<14;i++){//起点选择
			for(int j=0;j<14;j++){
				int start_value=0;
				if(core[i][j]==var){//我方点就是可移动点
					num++;//记录游戏是否结束
					...
					if(start_value>=max_start_value){//获得得分最大的起点
	...
	public List<CanWalkChess> CanWalkChessList_NO_AI(int var,int[][] core){
		...
		if(canFly_list.size()==0)return canFly_list;//列表为空，直接返回
	...
	public int CopyWalkMove(int var,int[][] core1,int copy_eat_num,int get_lian,int var1_yn,int var2_yn) throws ClassNotFoundException, IOException{
		...
		while(i<Walk_AI_STEP){//无限循环(限制步数，深度)
			...
			int x1,y1,x2,y2;
			if(var==1&&var1_yn==0)canWalk_list = CanWalkChessList_YES_AI(var,core);//白优随机
			if(var==2&&var2_yn==0)canWalk_list = CanWalkChessList_YES_AI(var,core);//黑优随机
			if(var==1&&var1_yn==1)canWalk_list = CanWalkChessList_NO_AI(var,core);//白劣随机
			if(var==2&&var2_yn==1)canWalk_list = CanWalkChessList_NO_AI(var,core);//黑劣随机
	...
	public CanWalkChess AI_walkChess(int var,int[][] core,int var1_yn,int var2_yn) throws ClassNotFoundException, IOException{
		...
		List<CanWalkChess> canwalk_list = new ArrayList<CanWalkChess>();
		if(var==1&&var1_yn==0)canwalk_list = get_YES_CanWalkChessList(var,core);//白优落点1111
		if(var==2&&var2_yn==0)canwalk_list = get_YES_CanWalkChessList(var,core);//黑优落点1111
		if(var==1&&var1_yn==1)canwalk_list = get_NO_CanWalkChessList(var,core);//白劣落点11111111
		if(var==2&&var2_yn==1)canwalk_list = get_NO_CanWalkChessList(var,core);//黑劣落点11111
		...
		for(int i=0;i<canwalk_list.size();i++){//遍历可落棋盘
			...
			for(int ai_num=0;ai_num<Walk_AI_NUM;ai_num++){
				int var_eatvalue = CopyWalkMove(n_var, copy_canwalk_core,copy_eat_num,get_lian,var1_yn,var2_yn);//模拟棋盘的函数111111111
				if(var==1){
					if(var_eatvalue>0)value++;
					if(value*1.0/Walk_AI_NUM>0.95){//防止满胜率无解情况
		...
		if(max_value<=10){//无最大落子情况
			if(get_NO_CanWalkChessList(var,core).size()>0){
				end_walk = get_NO_CanWalkChessList(var,core).get(0);
				System.out.println("无最大落子情况");
				System.out.print(var+"方，移动："+(end_walk.getX1()+1)+","+(char)(end_walk.getY1()+65)+"--至--"+(end_walk.getX2()+1)+","+(char)(end_walk.getY2()+65)+" ");
			}
		}
			...
			for(int j=0;j<14;j++){
				if(core[i][j]==n_var){//循环内为可吃的子
					...
					for(int ai_num=0;ai_num<EAT_AI_NUM;ai_num++){
						int var_eatvalue = CopyWalkMove(n_var, copy_caneat_core,0,0,var1_yn,var2_yn);//模拟棋盘的函数
						if(var==1){
							if(var_eatvalue>0)value++;
						}else{
							if(var_eatvalue<0)value++;
